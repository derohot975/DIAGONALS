...
- [x] **Performance**: FCP reale ~244ms, LCP ~260ms (SplashScreen 3s design-intenzionale).
- [x] **Design**: UI minimale stile Apple — coerente su tutte le schermate.
- [x] **UX Mobile**: Touch, haptic feedback, scrolling ottimizzato, safe areas.
...
- `T016`: `WineCard` + `WinesGrid` — dark glassmorphism, amber badge prezzo, testo white.
- `T017`: `PagellaHeader` sticky con bg dark, `PagellaEditor` textarea dark glass.

### Mobile & PWA
- `T015`: Safe area top (`env(safe-area-inset-top)`) su `#root`. Status bar `black-translucent` per PWA dark. Bottom già coperto da `--bottom-nav-total`.
...
---
*Ultimo aggiornamento: 25 Febbraio 2026 — Tutti i task completati.*
