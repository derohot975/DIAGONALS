import { WineEvent, InsertWineEvent, eventReports, wineEvents, wines, votes } from '@shared/schema';
import { db } from '../db';
...
export class EventStorage {
  ...
  async updateWineEvent(
    id: number,
    updates: Partial<InsertWineEvent>
  ): Promise<WineEvent | undefined> {
    const [event] = await db
      .update(wineEvents)
      .set(updates)
      .where(eq(wineEvents.id, id))
