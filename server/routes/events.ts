...
eventsRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Event creation error', 'EVENTS', error instanceof Error ? error : undefined);
    if (error instanceof z.ZodError) {
      res.status(400).json({ message: 'Invalid event data', errors: error.errors });
    } else {
      res.status(500).json({ message: 'Failed to create event', error: 'Internal error' });
    }
  }
});
...
eventsRouter.patch('/:id', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Event update error', 'EVENTS', error instanceof Error ? error : undefined);
    if (error instanceof z.ZodError) {
      res.status(400).json({ message: 'Invalid event data', errors: error.errors });
    } else {
      res.status(500).json({ message: 'Failed to update event', error: 'Internal error' });
    }
  }
});
...
eventsRouter.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Delete event error', 'EVENTS', error instanceof Error ? error : undefined);
    res.status(500).json({ message: 'Failed to delete event', error: 'Internal error' });
  }
});
    ...
    const results = wines
      .map((wine) => {
        ...
        const scores = wineVotes.map((vote) => {
          const score =
            typeof vote.score === 'number' ? vote.score : parseFloat(vote.score.toString());
...
eventsRouter.delete('/:eventId/participants/:userId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Remove participant error', 'EVENTS', error instanceof Error ? error : undefined);
