...
reportsRouter.get('/:id/pagella', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get pagella error', 'REPORTS', error instanceof Error ? error : undefined);
  ...
  try {
    ...
    const responseData = updatedPagella
      ? {
          ...
          updatedAt: updatedPagella.updatedAt
            ? new Date(updatedPagella.updatedAt).toISOString()
            : null,
    ...
    logger.error('Update pagella error', 'REPORTS', error instanceof Error ? error : undefined);
    return res.status(500).json({ error: 'Internal server error' });
  }
});
...
reportsRouter.post('/:id/complete', async (req: Request, res: Response) => {
  try {
    ...
    const wineResults: WineResultDetailed[] = wines
      .map((wine) => {
    ...
    const userRankings: UserRanking[] = participants
      .map((participant) => {
    ...
    logger.error('Event completion error', 'REPORTS', error instanceof Error ? error : undefined);
