...
async function performDatabaseWarmup(): Promise<{
  ok: boolean;
...
healthRouter.get('/health', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (status !== lastDbStatus) {
      logger.info('Health status transition', 'HEALTH', {
        from: lastDbStatus,
        to: status,
        callCount: healthCallCount,
      });
      lastDbStatus = status;
    }
    ...
    logger.error(
      `Unexpected /health error after ${errorDuration}ms`,
      'HEALTH',
