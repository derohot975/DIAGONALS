...
export function useAppHandlers({
  ...
  const handleChangeAdminPin = useCallback(
    (newPin: string) => {
  ...
  const handleCreateEvent = useCallback(
    (name: string, date: string, mode: string) => {
      if (!currentUser) {
  ...
  const handleUpdateEvent = useCallback(
    (id: number, name: string, date: string, mode: string) => {
  ...
  const handleProtectEvent = useCallback(
    (eventId: number, protect: boolean) => {
  ...
  const handleRegisterWine = useCallback(
    (wineData: {
      type: string;
      name: string;
      producer: string;
      ...
      if (appState.editingWine) {
        ...
        mutations.createWineMutation.mutate({
          ...shared,
  ...
  const handleVoteForWine = useCallback(
    (wineId: number, score: number, hasLode = false) => {
      if (!currentUser) return;
      const wine = wines.find((w) => w.id === wineId);
      if (!wine) return;
      mutations.voteMutation.mutate({
  ...
  const handleActivateVoting = useCallback(
  ...
  const handleDeactivateVoting = useCallback(
    async (eventId: number) => {
      try {
        await apiRequest('PATCH', `/api/events/${eventId}/voting-status`, {
