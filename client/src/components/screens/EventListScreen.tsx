import { Calendar, Edit3 } from '@/components/icons';
...
export default function EventListScreen({
  ...
  wines,
  votes: _votes,
  onShowEventDetails: _onShowEventDetails,
  onShowEventResults: _onShowEventResults,
  onShowAdmin,
  onShowHistoricEvents,

  onRegisterWine,
  onParticipateEvent,
  onVoteForWine: _onVoteForWine,
  onEditWine,
  ...
  const activeEvents = events.filter(
    (event) => event.status === 'registration' || event.status === 'active'
  );
    ...
    <div className="flex-1 flex flex-col bg-gradient-to-b from-[#300505] to-[#1a0303]">
      ...
      {currentUser && (
        <div className="flex-shrink-0 text-center pb-8">
          <p className="text-xs font-semibold text-white/30 uppercase tracking-[0.2em] mb-1">
            Benvenuto
          </p>
          <h2 className="text-3xl font-bold text-white tracking-tight">{currentUser.name}</h2>
        </div>
      )}
      ...
      <div
        ...
        style={{
          height:
            'calc(100dvh - 180px - var(--bottom-nav-total, 56px) - env(safe-area-inset-top, 0px))',
        }}
      >
