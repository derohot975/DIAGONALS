    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-shrink-0 px-6 pt-10 pb-6 flex flex-col items-center">
        <img
          src={diagoLogo}
          alt="Logo"
          className="w-16 h-auto mb-4 logo-filter drop-shadow-lg opacity-90"
        />
        <h2 className="text-lg font-black tracking-widest text-white/80 uppercase">
          Gestione Eventi
        </h2>
      </div>
            ...
            <div className="p-5">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    ...
                    className="p-4 bg-white/5 rounded-2xl flex items-center justify-center border border-white/10 active:scale-95 transition-all"
                  >
                    <Edit className="w-5 h-5 text-white/60" />
                  </button>
                  <button
                    onClick={() => handleDeleteConfirm(event)}
                    className="p-4 bg-red-500/10 rounded-2xl flex items-center justify-center border border-red-500/20 active:scale-95 transition-all"
                  >
                    <Trash2 className="w-5 h-5 text-red-400" />
                  </button>
                  <button
                    onClick={() => setShowParticipants((p) => ({ ...p, [event.id]: !p[event.id] }))}
                    className="p-4 bg-amber-500/10 rounded-2xl flex items-center justify-center border border-amber-500/20 active:scale-95 transition-all"
                  >
              ...
              {showParticipants[event.id] && (
                <div className="mt-5 pt-5 border-t border-white/10 space-y-2">
                  <p className="text-[10px] font-black text-white/30 uppercase tracking-widest text-center mb-3">
                    Partecipanti
                  </p>
                  {wines
                    ...
                    .map((wine) => (
                      ...
                      >
                        <span className="font-bold text-white/80 text-sm">
                          {users.find((u) => u.id === wine.userId)?.name || '...'}
                        </span>
                        <button
                          onClick={() =>
