          ...
          <div className="bg-white/5 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 p-6 shadow-2xl flex flex-col min-h-0">
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                  ...
                  {isLoginMode ? (
                    <div className="flex flex-col min-h-0">
                      ...
                      <div className="flex justify-center space-x-4 mb-4 flex-shrink-0">
                        {[0, 1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className={`w-3 h-3 rounded-full border-2 transition-all duration-200 ${pin.length > i ? 'bg-white border-white scale-110' : 'border-white/30'}`}
                          />
                        ))}
                      </div>
                    ...
                    <>
                      ...
                      <p className="text-white/30 text-xs text-center mt-3">
                        Usa giorno e mese di nascita — così te lo ricordi anche da ubriaco 🍷
                      </p>
                    </>
                  )}
                </div>
              </div>
              {/* chiusura flex-1 wrapper */}
              ...
              <button
                type="submit"
                disabled={
                  isLoading || pin.length !== 4 || (!isLoginMode && name.trim().length === 0)
                }
